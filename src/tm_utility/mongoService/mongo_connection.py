...
class MongoService:
    ...
    def save_or_append_user_history(
            self,
            user_id: str,
            messages: Union[str, Dict[str, Any], List[Union[str, Dict[str, Any]]]],
        ...
        """
        If session exists → append messages
        If session does not exist → create new session with messages
        """
        ...
        existing = self._get_collection(collection).find_one({
            "_id": session_id,
            "user_id": user_id
        })
        ...
        if existing:
            # Append path
            self._get_collection(collection).update_one(
            ...
            existing["messages"].extend(normalized_messages)
        ...
        self._get_collection(collection).insert_one(document)
        return document
